...
class Game {
    ...
    /** Stores moves from STORED. */
    void storedMoves(ArrayList<Move> stored) {
        if (stored == null) {
            storedMovesGame = new ArrayList<>();
        }
        for (int i = 0; i < 2; i++) {
            storedMovesGame.add(stored.get(i));
        }
    }
    ...
    /** Stores moves in an ArrayList format. */
    private ArrayList<Move> storedMovesGame = new ArrayList<>();
}
