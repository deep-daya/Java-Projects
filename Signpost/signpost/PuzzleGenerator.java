 ...
 *  @author Deep Dayaramani Paul Hilfinger
 */
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        int connectables = 0;
        ArrayList<Sq> connectSquares = new ArrayList<>();
        for (int i = 0;
             i < model.allSuccessors(start.pl, start.direction()).size(); i++) {
            Sq square =
                   model.get(model.allSuccessors(start.pl,
                           start.direction()).get(i));
            if (start.connectable(square)) {
                if (square.sequenceNum() == start.sequenceNum() + 1) {
                    return square;
                }
                connectables++;
                connectSquares.add(square);

            }
        }
        if (connectables == 1) {
            return connectSquares.get(0);
        }
        return null;
    }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        int connectables = 0;
        ArrayList<Sq> connectSquares = new ArrayList<>();
        for (int i = 0; i < end.predecessors().size(); i++) {
            Sq square = model.get(end.predecessors().get(i));
